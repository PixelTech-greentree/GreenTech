...
class _TreeCard extends StatelessWidget {
  final TreeModel tree; final int index;
  ...
  @override Widget build(BuildContext context) {
    return GestureDetector(
      onTap: () => Navigator.of(context).push(MaterialPageRoute(builder: (_) => TreeDetailScreen(treeId: tree.id))),
      child: Container(margin: const EdgeInsets.only(bottom: 16), decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(20), boxShadow: AppColors.cardShadow), child: Column(children: [
        Row(children: [
          Container(width: 90, height: 90, margin: const EdgeInsets.all(12), decoration: BoxDecoration(color: AppColors.softGreen, borderRadius: BorderRadius.circular(16)), child: ClipRRect(borderRadius: BorderRadius.circular(16), child: tree.latestPhotoUrl != null ? Image.network('${api.baseUrl}${tree.latestPhotoUrl}', fit: BoxFit.cover, errorBuilder: (_, __, ___) => Center(child: Text(TreePhases.getIcon(tree.phase), style: const TextStyle(fontSize: 40)))) : Center(child: Text(TreePhases.getIcon(tree.phase), style: const TextStyle(fontSize: 40))))),
          Expanded(child: Padding(padding: const EdgeInsets.symmetric(vertical: 12), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Text(tree.name ?? 'Daraxt #${index + 1}', style: const TextStyle(fontSize: 16, fontWeight: FontWeight.w700, color: AppColors.textDark)),
            const SizedBox(height: 6),
            Row(children: [_MiniChip(label: TreePhases.getName(tree.phase), color: AppColors.primaryGreen), const SizedBox(width: 6), _MiniChip(label: tree.status == 'active' ? 'Faol' : tree.status, color: tree.status == 'active' ? AppColors.success : AppColors.error)]),
            const SizedBox(height: 8),
            Row(children: [
              const Icon(Icons.calendar_today, size: 12, color: AppColors.textLight), const SizedBox(width: 4),
              Text(tree.plantedDateFormatted ?? '${tree.daysOld} kun', style: const TextStyle(fontSize: 11, color: AppColors.textLight)),
              if (tree.currentHealth != null) ...[const SizedBox(width: 12), Icon(Icons.favorite, size: 12, color: _healthColor(tree.currentHealth)), const SizedBox(width: 4), Text(tree.currentHealth == 'healthy' ? 'Sog\'lom' : tree.currentHealth!, style: TextStyle(fontSize: 11, color: _healthColor(tree.currentHealth)))],
            ]),
          ]))),
          const Padding(padding: EdgeInsets.only(right: 12), child: Icon(Icons.chevron_right, color: AppColors.textLight)),
        ]),
        if (tree.activeTasks.isNotEmpty || tree.pendingTasksCount > 0) Container(padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 12), decoration: BoxDecoration(color: AppColors.statusPending.withOpacity(0.08), borderRadius: const BorderRadius.vertical(bottom: Radius.circular(20))), child: Row(children: [const Icon(Icons.task_alt, size: 18, color: AppColors.statusPending), const SizedBox(width: 8), Expanded(child: Text('${tree.activeTasks.isNotEmpty ? tree.activeTasks.length : tree.pendingTasksCount} ta vazifa', style: const TextStyle(fontSize: 13, fontWeight: FontWeight.w600, color: AppColors.statusPending)))])),
      ])),
    );
  }
}
